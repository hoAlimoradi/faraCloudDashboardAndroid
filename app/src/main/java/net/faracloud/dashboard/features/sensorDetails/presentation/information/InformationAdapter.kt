package net.faracloud.dashboard.features.sensorDetails.presentation.information
...
class InformationAdapter(private val informations: List<String>) :  RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun getItemViewType(position: Int): Int =
        when (position) {
            0, 1, 2, 3, 5, 6, 7, 8, 9, 10, 11, 12 -> TYPE_TITLE
            4  -> TYPE_MAP
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) =
        when (holder.itemViewType) {
            TYPE_TITLE -> onBindTitle(holder, position)
            TYPE_MAP -> onBindMap(holder)
            else -> throw IllegalArgumentException()
    ...
    private fun onBindMap(holder: RecyclerView.ViewHolder) {
        val viewHolder = holder as MapViewHolder
